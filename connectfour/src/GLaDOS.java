...
public class GLaDOS implements IGameLogic {
    ...
    /**
     * Decides how many subsequent coins there are in the direction specified by
     * dx and dy. Does NOT examine the initial (x, y) position. A call to the
     * function starts in (x + dx, y + dy).
     */
    private static int subsequentCoins(int[][] board, int x, int y, int dx,
            int dy, int playerID) {
        int newX = x + dx;
        int newY = y + dy;
    ...
    /**
     * Tests whether the given player has one on a board with the given last
     * move (column and player).
     */
    private static Winner gameFinished(int[][] board, int lastMoveColumn) {
        ...
        if (lastMoveColumn != -1) {
            ...
            int row = 0;
            while (board[lastMoveColumn][row] == 0) {
                row++;
            }
            ...
            // Horizontal win
            // TODO Maybe end search if left is above 3, just to make it
            // terminate faster
            ...
            for (int[] aBoard : board) {
                // Not finished: One of the upper coins are not assigned to a
                // player (0)
                if (aBoard[0] == 0)
